...
public class NetworkManager extends CordovaPlugin {
    ...
    private String getType(NetworkInfo info) {
        if (info != null) {
            ...
            else if (type.equalsIgnoreCase(MOBILE) || type.equalsIgnoreCase(CELLULAR)) {
                type = info.getSubtypeName().toLowerCase();
                if (type.contains(GSM) ||
                        type.contains(GPRS) ||
                        type.contains(EDGE)) {
                    return TYPE_2G;
                }
                else if (type.contains(CDMA) ||
                        type.contains(UMTS) ||
                        type.contains(ONEXRTT) ||
                        type.contains(EHRPD) ||
                        type.contains(HSUPA) ||
                        type.contains(HSDPA) ||
                        type.contains(HSPA)) {
